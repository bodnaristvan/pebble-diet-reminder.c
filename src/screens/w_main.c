...
void main_window_load(Window *window) {
	...
	snprintf(calstrtemp, sizeof(calstrtemp), "Total calories: %d\nPress select to set daily value", persist_exists(PERSIST_CALORIES) ? (int)persist_read_int(PERSIST_CALORIES) : 2000);
	s_totalcal_layer = text_layer_create(GRect(0, 40, 144, 20));
	text_layer_set_overflow_mode(s_totalcal_layer, GTextOverflowModeWordWrap);
	text_layer_set_background_color(s_totalcal_layer, GColorClear);
	text_layer_set_text_color(s_totalcal_layer, GColorBlack);
	text_layer_set_text(s_totalcal_layer, calstrtemp);
	text_layer_set_font(s_totalcal_layer, fonts_get_system_font(FONT_KEY_GOTHIC_18));
	...
	s_nextcal_layer = text_layer_create(GRect(5, 60, 139, 50));
	text_layer_set_background_color(s_nextcal_layer, GColorClear);
	text_layer_set_overflow_mode(s_nextcal_layer, GTextOverflowModeWordWrap);
	text_layer_set_text_color(s_nextcal_layer, GColorBlack);
	text_layer_set_text(s_nextcal_layer, nxtstr);
	text_layer_set_font(s_nextcal_layer, fonts_get_system_font(FONT_KEY_GOTHIC_18));
	text_layer_set_text_alignment(s_nextcal_layer, GTextAlignmentLeft);
